const FullPortfolio = ({ setFullPortfolioHidden }) => {
    const modalInfo = [
        ...
        },
        {
            ...
            desc: 'A classic game gone digital: Connect Four.  This game is pure JavaScript; the board is created with the forEach method, adding an id and aria-label to each spot a puck can occupy.  After the seventh turn it uses the forEach method and a While loop to see if a player has won.  Using the coordinates of the last dropped puck, the game examines the surrounding spots for a puck of the same color; <strong>the coordinates for a win are not hard-coded in</strong>.  A small checkmark lets the players know whose turn it is, and a timer displays the game length.',
            img: '/connectFour.png',
        ...
        },
        {
            ...
            desc: 'Pure vanilla JavaScript meets Wordle.  It utilizes a free API to provide endless five letter words each day, no more waiting till tomorrow to get your next Wordle fix!  It uses if, if/else and forEach loops to build the board and control game functions.  The on-screen keyboard can be used with a mouse, or users can utilize their computer keyboard for an easier play.  The keyboard funcionality involves class-list and the attribute properties, if a letter is not in the puzzle, it is turned light grey after used in a guess.  If it is correct but in the wrong spot, it turns yellow, and correct letter in the correct spot results in a purple key.',
            img: '/wordlev2.png',
            demo: 'https://achulslander-wordle.netlify.app/',
            code: 'https://github.com/alleycaaat/Wordle',
            alt: 'A completed game of wordle that the player solved in three guesses.',
        },
        {
            id: 'notesKeeper',
            title: 'Notes Keeper - DEFUNCT',
            stack: 'ReactJS, FaunaDB, Netlify',
            desc: 'DEFUNCT - FaunaDB shut down, this project is no longer functional.  The code is still visible, but there is not a working demo.  This project has been updated to use appwrite.  See the next project for more information.  ORIGINAL DESCRIPTION: This project was created to understand how to build a Jamstack web-app with ReactJS, Netlify and FaunaDB.  It makes API calls to functions to compile, edit and delete the notes.  There is a loading screen that occurs when the API call to get all the notes is occuring to prevent load errors.',
            img: '/notesKeeper.png',
            demo: '',
        ...
        },
        {
            ...
            title: 'Notes Keeper',
            stack: 'ReactJS, appwrite, Vercel',
            desc: 'The original Notes Keeper has simply been updated to have a functional demo by moving to appwrite as the backend.  Guests have permissions to create, read, update and delete notes.  A loading screen shows when the front-end is communicating with appwrite.  Each note shows the date the note was created, or last updated if applicable.',
            img: '/notesKeeperAW.png',
            demo: 'https://achulslander-notes-keeper-aw.vercel.app/',
            code: 'https://github.com/alleycaaat/notes-keeper-aw',
            alt: 'a white background with three square, lavender notes.  Each has icons in the lower right corner to edit and erase the note, and the date the note was created in the lower left corner',
        },
        {
            id: 'racePace',
